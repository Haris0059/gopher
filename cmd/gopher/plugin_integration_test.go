...
// `gopher plugin list` runs successfully with no plugins installed.
func TestPluginSubcommand_ListEmpty(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("gopher plugin list failed: %v\n%s", err, out)
	}
...
func TestPluginSubcommand_ListAlias(t *testing.T) {
	...
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("gopher plugin ls failed: %v\n%s", err, out)
	}
...
// buildPluginBinary compiles the gopher binary into a temp directory and
