...
// buildBinary builds the gopher binary into a temp directory and returns
// its path. The caller is responsible for cleaning up the directory.
func buildBinary(t *testing.T) string {
	t.Helper()
	dir := t.TempDir()
	bin := filepath.Join(dir, "gopher")
...
// TestAutoModeSubcommand_Defaults verifies that `gopher auto-mode defaults`
...
// TestAutoModeSubcommand_Config verifies that `gopher auto-mode` and
// `gopher auto-mode config` both produce valid JSON output.
