...
| Doctor diagnostics | `pkg/doctor` (aggregator), `pkg/ui/doctor` (renderers), `pkg/ui/screens/doctor.go` (TUI) | `src/screens/Doctor.tsx`, `src/utils/doctorDiagnostic.ts` | **Done** | `gopher doctor` (`INST-03`) now runs the full-screen `DoctorModel` on a TTY and prints the same sections as plain text otherwise; the in-REPL `/doctor` slash command shares the same aggregator. All but two of `Collect`'s sections are backed by real collectors (agents via `skills.LoadAllAgents`, settings/keybinding/MCP parse warnings, PID version locks via new `installer.ListLocks`/`CleanupStaleLocks`, install-type/package-manager, dist tags, ripgrep, Linux glob footguns, `installer.CheckInstall()`). Gap: context warnings (CLAUDE.md size/agent-description/MCP-tool token totals, unreachable rules) and plugin-error reporting have no collector at all — see `TASKS.md` → `INST-07`, `INST-08` |
...
| ~~9~~ | ~~Doctor and setup-token TUI screens stubbed~~ Doctor fixed — `gopher doctor` launches the real `DoctorModel` TUI (plain-text fallback off a TTY); setup-token's `ConsoleOAuthFlow` remains a stand-in | `INST-03`, `INST-04` |
